...
async function createFleet(body: Record<string, unknown>): Promise<void> {
...
function CreatePanel({ tab, drivers, onCreated }: { tab: "vehicles" | "drivers" | "greeters"; drivers: Person[]; onCreated: () => void }) {
  const [open, setOpen] = useState(false); const [name, setName] = useState(""); const [phone, setPhone] = useState(""); const [supplierName, setSupplierName] = useState(""); const [plate, setPlate] = useState(""); const [model, setModel] = useState(""); const [size, setSize] = useState<"SMALL" | "LARGE">("SMALL"); const [driverId, setDriverId] = useState(""); const [saving, setSaving] = useState(false);
  const isVehicle = tab === "vehicles";
  async function submit() { setSaving(true); try { const role = tab === "drivers" ? "DRIVER" : "GREETER"; const vehicle = !isVehicle && tab === "drivers" && plate.trim() ? { plate, model, size } : undefined; await createFleet(isVehicle ? { type: "vehicle", plate, model, size, driverId, supplierName } : { type: "user", role, name, phone, supplierName, vehicle }); setOpen(false); setName(""); setPhone(""); setSupplierName(""); setPlate(""); setModel(""); setDriverId(""); onCreated(); } catch (error) { alert((error as Error).message); } finally { setSaving(false); } }
  return <div style={{ marginTop: 14 }}><button className="ev-btn" onClick={() => setOpen((value) => !value)}>{open ? "Formu kapat" : `+ Yeni ${isVehicle ? "araç" : tab === "drivers" ? "şoför" : "karşılamacı"} ekle`}</button>{open && <div className="ev-card" style={{ marginTop: 12, background: "rgba(14, 42, 52, 0.5)" }}><div className="ev-eyebrow">Yeni kayıt</div><div className="ev-field-grid" style={{ marginTop: 10 }}>{isVehicle ? <><div className="ev-field"><label className="ev-label">Plaka</label><input className="ev-input" value={plate} onChange={(e) => setPlate(e.target.value)} placeholder="07 EVT 99" /></div><div className="ev-field"><label className="ev-label">Model</label><input className="ev-input" value={model} onChange={(e) => setModel(e.target.value)} placeholder="Mercedes Vito" /></div><div className="ev-field"><label className="ev-label">Araç tipi</label><select className="ev-select" value={size} onChange={(e) => setSize(e.target.value as typeof size)}><option value="SMALL">Vito / Transporter</option><option value="LARGE">Sprinter</option></select></div><div className="ev-field"><label className="ev-label">Şoför</label><select className="ev-select" value={driverId} onChange={(e) => setDriverId(e.target.value)}><option value="">Şoför atanmamış</option>{drivers.filter((d) => d.active).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div></> : <><div className="ev-field"><label className="ev-label">Ad soyad</label><input className="ev-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ad Soyad" /></div><div className="ev-field"><label className="ev-label">Telefon</label><input className="ev-input" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+90..." /></div>{tab === "drivers" && <><div className="ev-field"><label className="ev-label">Araç plakası</label><input className="ev-input" value={plate} onChange={(e) => setPlate(e.target.value)} placeholder="07 EVT 99" /></div><div className="ev-field"><label className="ev-label">Araç modeli</label><input className="ev-input" value={model} onChange={(e) => setModel(e.target.value)} placeholder="Mercedes Vito" /></div><div className="ev-field"><label className="ev-label">Araç tipi</label><select className="ev-select" value={size} onChange={(e) => setSize(e.target.value as typeof size)}><option value="SMALL">Vito / Transporter</option><option value="LARGE">Sprinter</option></select></div></>}</>}</div><div className="ev-actions" style={{ marginTop: 12 }}><button className="ev-btn" disabled={saving} onClick={submit}>{saving ? "Kaydediliyor…" : "Kaydı oluştur"}</button></div></div>}</div>;
}
