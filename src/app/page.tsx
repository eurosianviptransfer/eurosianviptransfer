...
export default function HomePage() {
  ...
  return <main className="ev-home">
    ...
    <section className="ev-landing-hero ev-container">
      <div className="ev-hero-copy"><div className="ev-eyebrow"><span className="ev-pulse" />{t.eyebrow}</div><h1>{t.title.split("\n").map((line, index) => <span key={line}>{index > 0 && <br />}{line}</span>)}</h1><p>{t.lead}</p><div className="ev-actions"><Link href={bookingHref} className="ev-btn">{t.primary} <Arrow /></Link><Link href={`/takip?lang=${locale}`} className="ev-text-link">{t.secondary} <Arrow /></Link></div><div className="ev-hero-note"><span>✦</span>{t.note}</div></div>
